...
export function handleAddShips(ws: WebSocket, message: IncomingMessage) {
  ...
  const { ships, roomId } = message.data;
  ...
  if (!ships || !roomId) {
    return ws.send(JSON.stringify({
      type: 'error',
      data: { message: 'Missing ships or roomId' },
      id: 0,
    }));
  }

  console.log(`Adding ships to room ${roomId}:`, ships);
  ...
  const player = room.roomUsers.find(u => u.index === session.index);
  if (!player) {
    return ws.send(JSON.stringify({
      type: 'error',
